...
<head>
	<meta charset="UTF-8">
	<title>A Secure API using CodeIgniter 4!</title>
...
<body>
	...
	<header>
		...
		<div class="heroe">
			...
			<h1>A Secure API using CodeIgniter <?= CodeIgniter\CodeIgniter::CI_VERSION ?></h1>

			<h2>This project is built on top of Composer and Json Web Tokens(JWT). It has users and clients model. The user is authenticated against the user table data and is allowed the privilege of viewing the clients list with proper credentials.</h2>
	...
	<section>
		...
		<p>Download the project files and run the below command to install composer dependencies.</p>
		<pre><code>php composer.phar install</code></pre>

		<p>Import the Mysql DB from the below path to your mysql server.</p>
		<pre><code>db/ci-api-db.sql</code></pre>

		<p>Change the below code in the .env file with respect to mysql server.</p>
		<pre><code>database.default.hostname = localhost<br>
database.default.database = ci-api-db<br>
database.default.username = root<br>
database.default.password = <br>
database.default.DBDriver = MySQLi</code></pre>

		<p>Valid user credentials are in the below path.</p>
		<pre><code>db/sample-data.txt</code></pre>

		<p>In postman use the below link to get the access token by passing form-data under body using one of the credentials given above, as email and password.</p>
		<pre><code>http://localhost:8080/auth/login</code></pre>

		<p>With the token received in the above login, follow the below link with a authorization bearer token in the header.</p>
		<pre><code>http://localhost:8080/client</code></pre>

		<p>The token expires automatically after 1hr, you can also change this in the .env file by editing the line in milliseconds.</p>
		<pre><code>JWT_TIME_TO_LIVE=3600</code></pre>
